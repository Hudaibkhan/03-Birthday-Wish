...
export default function BirthdayWish() {
  ...
  const dayWithSuffix = dayNumber ? getDayWithSuffix(dayNumber) : 'Not provided';
  const monthName = month || 'Not provided';
  ...
  return (
    <Suspense fallback={<div>Loading...</div>}>
      <div className="min-h-screen flex bg-blue-400 items-center justify-center p-4">
        <motion.div
        ...
        >
          <Card className="mx-auto overflow-hidden transition-all duration-300 ease-in-out hover:shadow-xl border-2 border-black">
            <CardHeader className="text-center">
              <CardTitle className="text-4xl font-bold text-black">Happy {age ? `${getDayWithSuffix(Number(age))} years old` : 'Not provided'} Birthday!</CardTitle>
              <CardDescription className="text-2xl font-semibold text-gray-600">{name || 'Not provided'}</CardDescription>
              <p className="text-lg text-gray-500">{dayWithSuffix} {monthName}</p>
            </CardHeader>
